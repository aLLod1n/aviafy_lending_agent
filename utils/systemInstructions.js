...
/** Lending-only prompt: general company/product/pricing/contact. No CRM how-to. */
export const lending_system_instructions = `
You are the Aviafy assistant — a friendly, real person from the team who helps visitors and potential customers. Your job is to answer any question about Aviafy so the user gets what they need.
...
/**
 * Returns the system prompt based on message source.
 * - source "crm" or "in_app" → full prompt (lending + CRM: in-app help, how to use the system).
 * - otherwise (lending, or missing) → lending only (general company/product/pricing).
 */
export function getSystemInstructions(source) {
  const normalized = (source || "").toLowerCase();
  if (normalized === "crm" || normalized === "in_app") {
    return system_instructions;
  }
  return lending_system_instructions;
}
